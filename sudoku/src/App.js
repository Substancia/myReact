          ...
          onKeyEvent={(key) => {
            if (this.state.selectedSquare[0] !== null && this.state.selectedSquare[1] !== null) {
              switch(key) {
                ...
                break;
                case 'backspace': this.props.onChange(this.state.selectedSquare[0], this.state.selectedSquare[1], "");
                break;
                case 'del': this.props.onChange(this.state.selectedSquare[0], this.state.selectedSquare[1], "");
                break;
                default:
              }
...
class Game extends React.Component {
  ...
  componentDidMount() {
    ...
    fixed.map(index => squares[index[0]][index[1]] = index[2]);
    this.setState({
      fixed: fixed.map(index => [index[0], index[1]]),
      history: [{
        squares: squares,
      }],
    });
